...
export function AppShell({ active, onSelect, onBack, children, isLoading, onLoadDone }: Props) {
  ...
  const handleBack = useCallback(() => {
    if (exiting) return;
    setExiting(true);
    setTimeout(() => { setExiting(false); onBack(); }, 360);
  }, [exiting, onBack]);
  ...
  function toggleScreen() {
    ...
    if (screenOn) {
      setPowerAnim('off');
      setTimeout(() => { setScreenOn(false); setPowerAnim(null); }, 560);
    } else {
      setScreenOn(true);
      setPowerAnim('on');
          ...
          {isLoading ? (
            ...
            >
              <div className="section-header">
                ...
                <button type="button" className="section-back-btn" onClick={handleBack}>
                  ◀ Back
                </button>
              </div>
            ...
            <div className="modal-backdrop" onClick={() => setShowModal(false)}>
              <div className="modal-card" onClick={e => e.stopPropagation()}>
                ...
                <ul className="modal-steps">
                  <li><strong>Workshop</strong> - pick a format, upload art, export PDF</li>
                  <li><strong>My Covers</strong> - save and reopen your designs</li>
      ...
      <div className="tv-bottom-strip">
        <div className="tv-knob-group">
          <button
            ...
            data-glyph="?"
            onClick={() => { if (screenOn) setShowModal(true); }}
          />
          <button
            type="button"
            className="tv-knob tv-knob-btn"
        ...
        <div className="tv-brand">COVERWORKS</div>
